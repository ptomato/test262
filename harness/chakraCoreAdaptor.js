...
defines: [WScript, console, print, chakraCoreAdaptor]
---*/
...
var WScript = {
  ...
  LoadScriptFile(path) {
    if (!(knownScriptFiles.includes(path)))
      throw new Test262Error(`LoadScriptFile: Unhandled script file path ${path}`);
  },
  Platform: {
    BUILD_TYPE: "Release",
  },
...
const excMessageMatcher = /^(?:Range|Type)Error\s*:/;
...
var chakraCoreAdaptor = {
  verifyTest(expectedOutput) {
    ...
    // log should be empty or contain one line such as 'PASS' or 'Passed'
    if (!(["", "pass\n", "passed\n"].includes(log.toLowerCase())))
