...
TemporalHelpers.assertPlainYearMonth(
  date136206.add(months6),
  1362, 12, "M12", "add 6mo to Shahrivar 31 constrains to Esfand 30 in leap year",
  "ap", 1362, null);
...
TemporalHelpers.assertPlainYearMonth(
  ...
  1363, 12, "M12", "add 1y 2mo with result in the next year constrained to Esfand 29",
  "ap", 1363, null);
TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 1361, monthCode: "M10", calendar }, options).add(years1months2),
  1362, 12, "M12", "add 1y 2mo with result in the next year constrained to Esfand 30",
  "ap", 1362, null);

TemporalHelpers.assertPlainYearMonth(
  date136301.add(months1n),
  1362, 12, "M12", "subtract 1mo from Farvardin 31 constrains to Esfand 30 in leap year",
  "ap", 1362, null);
assert.throws(RangeError, function () {
...
TemporalHelpers.assertPlainYearMonth(
  ...
  1361, 12, "M12", "subtract 11mo with result in the previous year constrained to Esfand 29",
  "ap", 1361, null);
TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 1363, monthCode: "M11", calendar }, options).add(months11n),
  1362, 12, "M12", "subtract 11mo with result in the previous year constrained to Esfand 30",
  "ap", 1362, null);

TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 1364, monthCode: "M02", calendar }, options).add(years1months2n),
  1362, 12, "M12", "add 1y 2mo with result in the previous year constrained to Esfand 30",
  "ap", 1362, null);
TemporalHelpers.assertPlainYearMonth(
  Temporal.PlainYearMonth.from({ year: 1365, monthCode: "M02", calendar }, options).add(years1months2n),
  1363, 12, "M12", "add 1y 2mo with result in the previous year constrained to Esfand 29",
  "ap", 1363, null);
