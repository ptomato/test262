...
const knownFailures = {
  Array: [
    ...
    // May be failed by ChakraCore too under some circumstances? FIXME
    "FilterWithTypedArray.js",
    // Depends on particular toLocaleString output
    "toLocaleString.js",
  ],
};
...
const extraIncludes = [
  ['UnitTestFramework.js', 'chakraCoreUnitTestFramework.js'],
  ['memset_tester.js', 'chakraCoreMemsetTester.js'],
];

for (const { dir, intl402 = false } of dirsToProcess) {
  if (dir === '262' || dir === 'UnitTestFramework')
    continue;

  const targetDir = path.join(test262Dir, 'test', 'implementation-suites', 'ChakraCore', dir);
  ...
  for (const testElement of expectChildren(regressExeElement, 'test')) {
    ...
    const expectedOutput = testInfo.expectedOutput &&
      fs.readFileSync(path.join(chakraCoreDir, 'test', dir, testInfo.expectedOutput), UTF8)
        .replace(/\r/g, '');
    let testContents = fs.readFileSync(path.join(chakraCoreDir, 'test', dir, testInfo.file), UTF8)
      .replace(/\r/g, '');
    ...
    // Assume the presence of certain strings means certain auxiliary files are
    // being imported
    const includes = ['chakraCoreAdaptor.js'];
    if (expectedOutput)
      includes.push('compareArray.js');
    extraIncludes.forEach(([testString, include]) => {
      if (testContents.includes(testString))
    ...
    if (includes.includes('chakraCoreUnitTestFramework.js'))
